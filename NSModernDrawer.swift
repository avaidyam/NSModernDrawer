...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @IBOutlet weak var drawerView: NSView!
    private var drawer: NSDrawer! = nil
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        NSDrawer.modernize()
        
        self.drawer = NSDrawer(contentSize: NSSize(width: 250, height: 250), preferredEdge: .maxX)
        self.drawer.contentView = self.drawerView
        self.drawer.parentWindow = self.window
        self.drawer.resizableAxis = .horizontal
        self.drawer.open()
    }
}
...
public extension NSDrawer {
    ...
    public var drawerWindow: NSWindow? {
        return self.value(forKey: "_drawerWindow") as? NSWindow
    }
